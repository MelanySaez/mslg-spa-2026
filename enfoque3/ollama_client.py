...
def translate(prompt):
    ...
    Returns:
        El texto de respuesta del modelo (str).
    ...
    for attempt in range(1, config.OLLAMA_MAX_RETRIES + 1):
        try:
            ...
            last_error = e
            wait = 2 ** attempt
            logger.warning("Ollama error (attempt %d): %s — retrying in %ds", attempt, e, wait)
            time.sleep(wait)

    raise RuntimeError(f"Ollama falló tras {config.OLLAMA_MAX_RETRIES} intentos: {last_error}")
