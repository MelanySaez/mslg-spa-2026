"""Orquestador enfoque7.4 — réplica de enfoque7.2 (reverso MSLG→SPA) con Ollama.

Mismo dispatch (`few_shot_rag_curriculum_reverse`, k=10) que 7.2, pero
llamando a `ollama_client.translate(system, user)`. Mantiene la generación
del archivo de submission oficial `TeamName_SolutionName_MSLG2SPA.txt` y la
métrica COMET (vía CLI `comet-score`) cuando ENABLE_COMET=true.
"""
...
def run_experiment(experiment: dict, pool: list, val: list, emb_index=None):
    ...
    if metrics.get("comet") is not None:
        ...
        print("  COMET:  N/A (la actividad la calcula sobre el .txt enviado)")
...
def run_all(experiments=None):
    ...
    if need_embeddings:
        ...
        print("\nConstruyendo índice de embeddings sobre MSLG (rag/rag-curriculum)...")
...
def _save_submission_txt(results: list, exp_name: str):
    """Genera archivo .txt en formato oficial MSLG-SPA 2026 (subtask MSLG2SPA)."""
    os.makedirs(config.RESULTS_DIR, exist_ok=True)
...
def run_submission(experiments=None):
    """Inferencia sobre el test set oficial (MSLG2SPA_test.txt) vía Ollama.

    Diferencias vs run_all:
      - Pool RAG = TODO el train (489 pares), no el split de 400.
      - No hay gold SPA, así que no se computan métricas (COMET la calcula
        la actividad sobre el .txt enviado).
    ...
    if need_embeddings:
        import embedding_index as emb_mod
        print("\nConstruyendo índice de embeddings sobre MSLG (pool train completo)...")
        emb_index = emb_mod.EmbeddingIndex(pool)
    ...
    for experiment in experiments:
        ...
        for i, item in enumerate(test_items):
            ...
            raw = ollama_client.translate(system_prompt, user_prompt)
...
def _print_summary(summary: list):
    print(f"\n{'='*70}")
    print(f"  RESUMEN — ENFOQUE 7.4 reverso MSLG2SPA (Ollama: {config.OLLAMA_MODEL})")
    print(f"{'='*70}")
