"""Orquestador enfoque7.6 — réplica de enfoque7.2 (reverso MSLG→SPA) con NVIDIA NIM.

Mismo dispatch (`few_shot_rag_curriculum_reverse`, k=10) que 7.2, pero
llamando a `nvidia_client.translate(system, user)` contra el endpoint
hospedado de NVIDIA Build. Mantiene la generación del archivo de submission
oficial `TeamName_SolutionName_MSLG2SPA.txt` y la métrica COMET (vía CLI
`comet-score`) cuando ENABLE_COMET=true.
"""
...
def run_experiment(experiment: dict, pool: list, val: list, emb_index=None):
    ...
    print(f"  Modelo (NVIDIA NIM): {config.NVIDIA_MODEL}")
    ...
    for i, par in enumerate(val):
        ...
        raw_response = nvidia_client.translate(system_prompt, user_prompt)
...
def _print_summary(summary: list):
    print(f"\n{'='*70}")
    print(f"  RESUMEN — ENFOQUE 7.6 reverso MSLG2SPA (NVIDIA NIM: {config.NVIDIA_MODEL})")
    print(f"{'='*70}")
