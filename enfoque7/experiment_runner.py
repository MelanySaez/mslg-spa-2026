"""Orquestador de experimentos SPA→MSLG — Enfoque 7 (Anthropic API).

Solo experimentos zero-shot y few-shot. El sistema de reglas va en el bloque
`system` y se cachea entre requests del mismo experimento, reduciendo el costo
de input repetido ~90 % en requests subsecuentes.
"""
...
def _print_summary(summary: list):
    print(f"\n{'='*60}")
    print(f"  RESUMEN — ENFOQUE 7 ({config.ANTHROPIC_MODEL})")
    print(f"{'='*60}")
    print(f"  {'Experimento':<24} {'BLEU':>8} {'METEOR':>8} {'chrF':>8}")
    print(f"  {'-'*24} {'-'*8} {'-'*8} {'-'*8}")
    for row in summary:
        print(
            f"  {row['name']:<24} {row['bleu']:>8.4f} "
